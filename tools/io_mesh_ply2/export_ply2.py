...
def save_mesh(filepath,
    ...
    for face in mesh.polygons:
        ...
        for vert in [mesh.loops[i] for i in face.loop_indices]:
            ...
            position = mesh_verts[vidx].co[:]

            #fw(" %.6f %.6f %.6f" % (position[0], position[1], position[2]))
            #fw(" %.6f %.6f %.6f" % (normal[0], normal[1], normal[2]))
            #fw(" %.6f %.6f" % (uv[0], uv[1]))
            fw("%.6f %.6f %.6f" % (tangent[0], tangent[1], tangent[2]))
            fw(" %.6f %.6f %.6f" % (bitangent[0], bitangent[1], bitangent[2]))
            fw("\n")
