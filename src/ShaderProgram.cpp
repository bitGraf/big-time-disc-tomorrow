...
/*void ShaderProgram::BuildShader(std::string vShaderPath,
	std::string fShaderPath, const std::string &name) {
	Name = name;
	printf("Building shader(%s): [%s]|[%s]\n", name.c_str(), vShaderPath.c_str(), fShaderPath.c_str());
	//Load Shaders
	...
	{
		// open files
		vShaderFile.open(ShaderResourcePath + vShaderPath);
		//std::cout << "1" << std::endl;
		fShaderFile.open(ShaderResourcePath + fShaderPath);
		//std::cout << "2" << std::endl;
	...
	{
		std::cout << "ERROR::SHADER::FILE_NOT_SUCCESFULLY_READ [" << vShaderPath  << ", " << fShaderPath << "]" << std::endl;
	}
	...
	if (!success) {
		glGetShaderInfoLog(vShader, 512, NULL, infoLog);
		std::cout << "ERROR::SHADER::VERTEX::COMPILATION_FAILED(" << vShaderPath << ")\n" << infoLog << std::endl;
	}
	//Fragment shader
	GLuint fShader;
	fShader = glCreateShader(GL_FRAGMENT_SHADER);
	...
	if (!success) {
		glGetShaderInfoLog(fShader, 512, NULL, infoLog);
		std::cout << "ERROR::SHADER::FRAGMENT::COMPILATION_FAILED(" << fShaderPath << ")\n" << infoLog << std::endl;
	}
	//Shader program
	shaderID = glCreateProgram();
	glAttachShader(shaderID, vShader);
